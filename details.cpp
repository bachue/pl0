...
#define DECLARE_NEW_TEMPVARNAME_TO_OPER3 \
{\
	newcodenode.oper3=this->tempvar.next();\
	this->declare_new_var(newcodenode.oper3);\
}

SyntacticAnalyzer::SyntacticAnalyzerResult SyntacticAnalyzer::condition(LexicalAnalyzer& lexical_analyzer,const std::string& point,bool& value)
{
	SyntacticAnalyzer::SyntacticAnalyzerResult res=RIGHT,tmp_res=RIGHT,tmp_res2=RIGHT;
	int tmp_value,tmp_value2,type=lexical_analyzer.peeksymtype();
    std::string oper1,word;
    if(type==ODDTYPE)
	{
		...
		{
			value=tmp_value%2==1;
			if(value==false)//如果value为假，证明中间所有语句均不可能执行到，直接跳过
			{
				this->intermediate_code_deque_vector[this->cur_procedure_index].push_back(
				SyntacticAnalyzer::IntermediateCodeNode(JUMP,point));
			}
		}
		else
		{
			this->intermediate_code_deque_vector[this->cur_procedure_index].push_back(
				SyntacticAnalyzer::IntermediateCodeNode(ODDSYM,this->tempvar.prev_temp_varname,point));
		}
		return res;
	}
    else
    {
		...
		else oper1=this->tempvar.prev_temp_varname;//记录oper1的返回内容

        if(type=lexical_analyzer.getsymtype(),word=lexical_analyzer.getprevsymword(),
           type!=EQLTYPE && type!=NEQTYPE && type!=LSSTYPE && type!=LEQTYPE && type!=GTRTYPE && type!=GEQTYPE)
		{
		...
		else
		{
			...
			else if(tmp_res==VALUE && tmp_res2==VALUE)
			{
				switch(type)
				{
				case EQLTYPE: value=tmp_value==tmp_value2;break;
				case NEQTYPE: value=tmp_value!=tmp_value2;break;
				case LSSTYPE: value=tmp_value<tmp_value2;break;
				case LEQTYPE: value=tmp_value<=tmp_value2;break;
				case GTRTYPE: value=tmp_value>tmp_value2;break;
				case GEQTYPE: value=tmp_value>=tmp_value2;break;
				}
			...
			{
				this->intermediate_code_deque_vector[this->cur_procedure_index].push_back(
					SyntacticAnalyzer::IntermediateCodeNode(word,oper1,this->tempvar.prev_temp_varname,point));
				//这里直接依赖“this->tempvar.prev_temp_varname中存储返回值的字符串”
			}
...
SyntacticAnalyzer::SyntacticAnalyzerResult SyntacticAnalyzer::expression(LexicalAnalyzer& lexical_analyzer,int& value)
{
	...
	for(;;)
	{
		...
		if(type==PLUSTYPE || type==MINUSTYPE)
		{
			if(type==MINUSTYPE) fac=-1;
			lexical_analyzer.getsymtype();
		}
		if(!(res=item(lexical_analyzer,value))) return ERROR;
		else if(res==VALUE && value==0 && (lexical_analyzer.peeksymtype()==PLUSTYPE || lexical_analyzer.peeksymtype()==MINUSTYPE)) continue;
		...
		else if(fac==-1 && res==RIGHT)//如果这是个变量且在前面加过负号，在这里将出现（times -1 var tempvar）的中间代码
		{
			...
			DECLARE_NEW_TEMPVARNAME_TO_OPER3
    ...
    while(type=lexical_analyzer.peeksymtype(),type==PLUSTYPE || type==MINUSTYPE)
    {
		...
		{
			if(type==PLUSTYPE) value+=tmp_value;
			else /*if(type==MINUSTYPE)*/ value-=tmp_value;
		...
		else
		{
			...
			if(res==VALUE /* && tmp_res==RIGHT*/)
			{
				...
				newcodenode.command=(type==PLUSTYPE?PLUSOPER:MINUSOPER);//PLUS OR MINUS
				...
				else DECLARE_NEW_TEMPVARNAME_TO_OPER3
			...
			{
				newcodenode.command=(type==PLUSTYPE?PLUSOPER:MINUSOPER);//不是PLUS就是MINUS
				newcodenode.oper1=oper1;
...
SyntacticAnalyzer::SyntacticAnalyzerResult SyntacticAnalyzer::item(LexicalAnalyzer& lexical_analyzer,int& value)
{
	SyntacticAnalyzer::SyntacticAnalyzerResult res=RIGHT,tmp_res=RIGHT;
	SyntacticAnalyzer::IntermediateCodeNode newcodenode;
	int tmp_value,some_value_to_optimize=1/*乘除法这里是1,加减法是0*/,type;
	...
	for(;;)
	{
		...
		else if(res==VALUE && value==1 && lexical_analyzer.peeksymtype()==TIMESTYPE)
		{
			lexical_analyzer.getsymtype();
			continue;
		}
    ...
    while(type=lexical_analyzer.peeksymtype(),type==TIMESTYPE || type==SLASHTYPE)
    {
		...
		{
			if(type==TIMESTYPE) value*=tmp_value;
			else/* if(type==SLASHTYPE)*/ value/=tmp_value;
			sprintf(buffer,"%d",value);
		...
		else
		{
			...
			if(res==VALUE /*&& tmp_res==RIGHT*/)
			{
				...
				newcodenode.command=(type==TIMESTYPE?TIMESOPER:SLASHOPER);//TIMES OR SLASH
				newcodenode.oper1=oper1;
				newcodenode.oper2=this->tempvar.prev_temp_varname;
				if(SyntacticAnalyzer::TempVar::isTempVar(this->tempvar.prev_temp_varname)) newcodenode.oper3=this->tempvar.prev_temp_varname;
				else DECLARE_NEW_TEMPVARNAME_TO_OPER3
...
SyntacticAnalyzer::SyntacticAnalyzerResult SyntacticAnalyzer::factor(LexicalAnalyzer& lexical_analyzer,int& value)
{
   ...
   int type=lexical_analyzer.getsymtype();
	...
	else if(type==IDENTTYPE)
	{
		if((iter=this->ident_stack_find(word.c_str(),QUERY_ALL_PROC))==this->ident_stack.end())
		{
	...
	else /*if(type==LPAREN)*/
	{
		...
		if(lexical_analyzer.getsymtype()!=RPARENTYPE)
		{
