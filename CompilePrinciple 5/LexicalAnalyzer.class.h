...
private:
    ...
    std::deque<std::string> symtypes;
    std::deque<std::string> symwords;
    std::deque<int> symnumlines;
    std::deque<int> symnumchars;
    int curdequeindex;
    bool resetmode;
public:
    bool open(const std::string &path="");
    ...
    void reset();
private:
    std::string getKeyword(std::string) const;
    std::string get_operator_name(const std::string&) const;
	bool getsym(std::string&,std::string&);
};
